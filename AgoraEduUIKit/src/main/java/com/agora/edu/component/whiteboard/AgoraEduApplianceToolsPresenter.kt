    ...
    fun setApplianceImage(resId: Int?) {
        resId?.let {
            if (isNeedUseSetColor()) {
                ...
                val tint = hotfixTintColorForAcadsoc(binding.root.context, config.activeAppliance, defaultTintColor)
                AgoraEduImageUtils.setImageTintResource(resId, tint, binding.optionItemAppliance)
            }
        }
    }

    /**
     * if select appliance is Clicker、Select、Eraser, will not set tint.
     */
    private fun hotfixTintColorForAcadsoc(context: Context, type: WhiteboardApplianceType, tint: Int): Int {
        if (type == WhiteboardApplianceType.Clicker || type == WhiteboardApplianceType.Select
            || type == WhiteboardApplianceType.Eraser) {
            return ContextCompat.getColor(context, R.color.acadsoc_theme_color_third)
        }
        return tint
    }
