...
public class BoardRoomImpl extends NetlessManager<Room> implements BoardRoom, RoomListener {
    ...
    public void setWritable(boolean writable) {
        if (t != null) {
            ...
            t.setWritable(writable, new Promise<Boolean>() {
                @Override
                public void then(Boolean aBoolean) {
                    Constants.INSTANCE.getAgoraLog().i(TAG + "->0setWritable-then:" + aBoolean);
                    if (aBoolean) {
                        // restore memberState
                        t.setMemberState(memberState);
    ...
    public void setWritable(boolean writable, Promise<Boolean> promise) {
        if (t != null) {
            ...
            t.setWritable(writable, new Promise<Boolean>() {
                @Override
                public void then(Boolean aBoolean) {
                    Constants.INSTANCE.getAgoraLog().i(TAG + "->1setWritable-then:" + aBoolean);
                    if (aBoolean) {
                        // restore memberState
                        t.setMemberState(memberState);
                        disableSerialization(false);
    ...
    void onFail(SDKError error) {
        ...
        Constants.INSTANCE.getAgoraLog().e(TAG + "->onFail:" + errorMsg);
        ...
        for (String errorInfo : errorMessage) {
            if (errorMsg.contains(errorInfo) && joinFailedRetry > 0) {
                ...
                Constants.INSTANCE.getAgoraLog().w(TAG, "->joinRoom-retry-sdkInitFailed");
